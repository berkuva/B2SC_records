...
# Remove infinite values from the data
...
#top_variable_genes = np.argsort(gene_cv)[::-1][0:100]
...
# Print max value out of all values in gen_top
# Print how many columns are all zeroes
print("Number of columns that are all zeroes:", np.sum(~gen_top.any(axis=0)))
# Calculate Pearson correlation coefficient
...
pearson_corr = np.corrcoef(adata_control, rowvar=False)
