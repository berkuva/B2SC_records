...
pbmc.data <- Read10X(data.dir = "/Users/hyunjaecho/Downloads/raw_feature_bc_matrix/")
# Initialize the Seurat object with the raw (non-normalized data).
gene_expression_matrix <- pbmc.data$`Gene Expression`
pbmc <- CreateSeuratObject(counts = gene_expression_matrix, project = "pbmc10k", min.cells = 3, min.features = 200)
...
barcodes <- read.table("/Users/hyunjaecho/Downloads/raw_feature_bc_matrix/barcodes.tsv", header = TRUE, sep = "\t")
...
# Save as a CSV file
write.table(mapping, file = "barcode_to_celltype.csv", sep = ",", row.names = FALSE, quote = FALSE)

# Or save as a TSV file
write.table(mapping, file = "barcode_to_celltype.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
